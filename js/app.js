// WingChun V23.0 - modular app with data-layer-compatible video fields
let currentLang='zh';
...
function normalize(v){ const ep=Number(v.episode||v.id||0); return {...v,episode:ep,title:v.title||v.name||`第 ${ep} 集`,titleEn:v.titleEn||v.title_en||v.englishTitle||titleEnMap[ep]||'',url:v.url||v.youtube||v.link||'#',category:v.category||'其他',categoryEn:v.category_en||catEn[v.category]||v.category||'Others'}; }
...
function renderLatest(){ const v=videos[0]; if(!v) return; document.getElementById('latestThumb').href=v.url; document.getElementById('latestWatch').href=v.url; document.getElementById('latestImg').src=thumb(v); document.getElementById('latestEpisode').textContent=currentLang==='en'?`Episode ${v.episode}`:`第 ${v.episode} 集`; document.getElementById('latestCategory').textContent=currentLang==='en'?(v.categoryEn||catEn[v.category]||v.category):v.category; document.getElementById('latestTitle').innerHTML=titleFor(v,true); }
