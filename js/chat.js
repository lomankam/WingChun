...
(function(){
  ...
  const AZHI_WORKER_URL='https://wingchun-sync.201505-login.workers.dev/api/ai/chat';
  ...
  async function buildAzhiPrompt(userText){
    ...
    return [
      ...
      '回答規則：',
      '1. 請使用使用者本次提問的主要語言回答；中文一律使用繁體中文，不要因網站資料是中文而改用中文。',
      ...
      '8. 語氣像熟悉網站的真人助教，自然、親切、有耐心，不要像制式 FAQ。先用一至兩句直接回答，再依需要補充；只有內容較多時才使用條列，不要每次套用同一種格式。',
  ...
  async function sendAzhiReply(userText, chatRefForWrite, sourceMessageId){
    ...
    try{
      ...
      const reply=(data && data.reply ? String(data.reply) : '我是阿智，目前暫時無法回答，請稍後再試。').slice(0,5000);
